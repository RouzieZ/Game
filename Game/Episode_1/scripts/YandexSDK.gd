...
func init_sdk() -> bool:
	...
	var bridge_exists: Variant = _eval("Boolean(window.pzYandexBridge && window.pzYandexBridge.init);")
	if not bool(bridge_exists):
		return false
...
func show_rewarded_ad(callback: Callable = Callable()) -> void:
	...
	if callback.is_valid():
		# Reward confirmation requires a JavaScript callback and is intentionally disabled in the stub.
		callback.call(false)
