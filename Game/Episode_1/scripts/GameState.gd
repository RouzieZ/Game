...
const DEFAULT_INVENTORY: Array[String] = ["phone", "keys_home"]
...
const DEFAULT_FLAGS: Dictionary = {
	...
	"hero_alone": false,
	"military_obeyed": false,
	"final_route_highway": false,
	"final_route_forest": false,
	"final_route_industrial": false,
	"final_route_railway": false,
	"ending_route": "",
	"camp_talk_danya": false,
	"camp_talk_lera": false,
	"camp_talk_mila": false
}
...
func from_dictionary(data: Dictionary) -> void:
	...
	if typeof(inventory_value) == TYPE_ARRAY:
		for item_id in inventory_value:
			var loaded_item_id: String = str(item_id)
			if has_item_data(loaded_item_id) and get_inventory_weight() + get_item_weight(loaded_item_id) <= INVENTORY_LIMIT:
				inventory.append(loaded_item_id)
